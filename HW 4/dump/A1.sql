...
SELECT ANAME, AID, sum(timetofeed)
from animal
WHERE TIMETOFEED in (select timetofeed from animal where acategory = 'rare')
group by ANAME, AID;
...
SELECT H.ZOOKEEPID,SUM(A.TIMETOFEED) AS TOTALFEEDTIME
FROM HANDLES H, ANIMAL A, ZOOKEEPER Z
WHERE A.AID = H.ANIMALID AND Z.ZID =H.ZOOKEEPID 
GROUP BY H.ZOOKEEPID;
